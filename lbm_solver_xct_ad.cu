...
#define PART 0 //left 0 or right 1
#define SCALE_V 0.15
#define SCALE_C 1.2
...
__global__ void boundary_block(float* vel, int* mask, float* rho, float* rho_c, float* f, float* f_c, float* w, float* e) {
	...
	if (mask[y * m + x] != 0 && x > 0 && x < m - 1 && y>0 && y < n - 1) {
		...
		//if (react_value < 0) {
		//	vel[y * m * 2 + x * 2 + 0] = (abs(vel[y * m * 2 + x * 2 + 0]) + 0.0001) * vecx * react_value;
		//	vel[y * m * 2 + x * 2 + 1] = (abs(vel[y * m * 2 + x * 2 + 1]) + 0.0001) * vecy * react_value;
		//}
		//else {
		//	vel[y * m * 2 + x * 2 + 0] = (abs(vel[y * m * 2 + x * 2 + 0]) * 0 + 0.01) * vecx * react_value;
		//	vel[y * m * 2 + x * 2 + 1] = (abs(vel[y * m * 2 + x * 2 + 1]) * 0 + 0.01) * vecy * react_value;
		//}
...
__global__ void rho_to_bitmap(unsigned char* bitmap, float* rho) {
	int x = threadIdx.x + blockIdx.x * blockDim.x;
	int y = threadIdx.y + blockIdx.y * blockDim.y;
	int offset = x + (y+n) * m*2;
	float mag = (rho[x+y*m]) / SCALE_C;
	mag = min(1.0, mag);
	/*float vor = vel_gradient(vel,x,y);
	float mag = (vor+0.02) / 0.02;*/
	if (mag >= 0.5 && mag <= 1) {
...
__global__ void vel_ave_to_bitmap(unsigned char* bitmap, float* vel) {
	...
	float mag = sum / n / SCALE_V;
	mag = min(1.0, mag);
	for (int y = 0; y < n * mag; y++) {
		bitmap[(y+2*n) * m * 2 * 4 + x * 4 + 0] = 0;
		bitmap[(y + 2 * n) * m * 2 * 4 + (x + m) * 4 + 1] = 255;
		bitmap[(y + 2 * n) * m * 2 * 4 + (x + m) * 4 + 2] = 255;
		bitmap[(y + 2 * n) * m * 2 * 4 + (x + m) * 4 + 3] = 255;
	}
}
__global__ void rho_ave_to_bitmap(unsigned char* bitmap, float* rho) {
	...
	float mag = sum/n/ SCALE_C;
	mag = min(1.0, mag);
	for (int y = 0; y < n*mag; y++) {
		bitmap[(y + n) * m * 2 * 4 + (x + m) * 4 + 0] = 255;
		bitmap[(y + n) * m * 2 * 4 + (x + m) * 4 + 1] = 255;
		bitmap[(y + n) * m * 2 * 4 + (x + m) * 4 + 2] = 0;
		bitmap[(y + n) * m * 2 * 4 + (x + m) * 4 + 3] = 255;
	}
}
__global__ void rho_y_to_bitmap(unsigned char* bitmap, float* rho) {
	...
	float mag = sum/m / SCALE_C;
	mag = min(1.0, mag);
	for (int x = 0; x < m * mag; x++) {
		bitmap[y * m * 2 * 4 + (x + m) * 4 + 0] = 255;
		bitmap[y * m * 2 * 4 + (x + m) * 4 + 1] = 0;
		bitmap[y * m * 2 * 4 + (x + m) * 4 + 2] = 255;
		bitmap[y * m * 2 * 4 + (x + m) * 4 + 3] = 255;
	}
}
...
int main() {
	...
	DataBlock data;
	CPUAnimBitmap bitmap((int)m*2, (int)n*3, (void*)&data);
	data.bitmap = &bitmap;
	data.frames = 0;
	rho = new float[m * n];
	f = new float[m * n * 9];
	...
	if (ENABLEDISPLAY) {
		...
		GifWriter writer = {};
		GifBegin(&writer, filename.data(), 2 * m, 3 * n, 20, 8, true);
		unsigned char* image;
		image = new unsigned char[2 * m * 3 * n * 4];
		unsigned char* image_rev;
		image_rev = new unsigned char[2 * m * 3 * n * 4];
		for (int frame = 0; frame < 200; ++frame)
		{
			...
			for (int y = 0; y < 3 * n; y++) {
				for (int x = 0; x < 2 * m; x++) {
					image_rev[(y * 2 * m + x) * 4 + 0] = image[((3 * n - y - 1) * 2 * m + x) * 4 + 0];
					image_rev[(y * 2 * m + x) * 4 + 1] = image[((3 * n - y - 1) * 2 * m + x) * 4 + 1];
					image_rev[(y * 2 * m + x) * 4 + 2] = image[((3 * n - y - 1) * 2 * m + x) * 4 + 2];
					image_rev[(y * 2 * m + x) * 4 + 3] = image[((3 * n - y - 1) * 2 * m + x) * 4 + 3];
				}
			}

			GifWriteFrame(&writer, image_rev, 2 * m, 3 * n, 2, 8, true);
		}
		GifEnd(&writer);
	}
