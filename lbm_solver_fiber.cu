...
__global__ void boundary_fiber(float* vel, int* mask,float* rho,float* f,float* w,float* e) {
	int x = threadIdx.x + blockIdx.x * blockDim.x;
	int y = threadIdx.y + blockIdx.y * blockDim.y;
	if (mask[y * m + x] !=0) {
		vel[y * m * 2 + x * 2 + 0] = 0.0;
...
void anim_gpu(DataBlock* d, int ticks) {
	...
	for (int k = 0; k < nstep; k++) {
		...
		cudaDeviceSynchronize();
		boundary_fiber << <grid_fiber, threads >> > (d->dev_vel, d->dev_mask, d->dev_rho, d->dev_f, d->dev_w, d->dev_e);
		cudaDeviceSynchronize();
	}
